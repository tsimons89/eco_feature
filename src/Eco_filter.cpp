...
Mat Eco_filter::apply(const vector<Mat>& images){
	Mat ret_images;
...
Mat Eco_filter::apply(Mat image){
	this->image = image;
...
void Eco_filter::diff_x(){
	Size new_size(image.cols-1,image.rows);
	image = image(Rect(Point(1,0),new_size)) - image(Rect(Point(0,0),new_size));
	image /=2;
}

void Eco_filter::diff_y(){
	Size new_size(image.cols,image.rows-1);
	image = image(Rect(Point(0,1),new_size)) - image(Rect(Point(0,0),new_size));
	image /=2;
}

void Eco_filter::blur_x(int n){
	for(int i = 0; i < n; i++)
		blur_x();
}

void Eco_filter::blur_y(int n){
	for(int i = 0; i < n; i++)
		blur_y();
}

void Eco_filter::diff_x(int n){
	for(int i = 0; i < n; i++)
		diff_x();
}

void Eco_filter::diff_y(int n){
	for(int i = 0; i < n; i++)
		diff_y();
}
