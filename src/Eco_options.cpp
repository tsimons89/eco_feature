...
void Eco_options::add_train_options(){
		...
		("gene_max,g", po::value<unsigned int>()->default_value(10),"Max value for any gene in genome")
...
void Eco_options::parse_train_options(int argc,char * argv[]){        
	...
	Eco_population::set_population_size(vm["population_size"].as<int>());
	Adaboost_model::set_pool_size(vm["pool_size"].as<unsigned int>());
	Eco_population::set_mutation_rate(vm["mutation_rate"].as<double>());
	Eco_population::set_replacement_rate(vm["replacement_rate"].as<double>());
	Eco_population::set_percent_of_images_held(vm["percent_images_held"].as<double>());
	Eco_genome::set_gene_max(vm["gene_max"].as<unsigned int>());
	Eco_creature::set_num_trees(vm["num_trees"].as<unsigned int>());
	Eco_creature::set_max_tree_depth(vm["tree_depth"].as<unsigned int>());
}
